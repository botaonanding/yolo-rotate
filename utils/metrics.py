...
def fitness(x):
    ...
    w = [0.0, 0.0, 0.1, 0.9]  # weights4 for [P, R, mAP@1.0, mAP@1.0:0.95]
    return (x[:, :4] * w).sum(1)
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir='.', names=()):
    ...
    Source: https://github.com/rafaelpadilla/Object-Detection-Metrics.
        :params tp(correct): [pred_sum, 10]=[1905, 10] bool 整个数据集所有图片中所有预测框在每一个iou条件下(1.0~0.95)10个是否是TP
    ...
    for ci, c in enumerate(unique_classes):
        ...
        if n_p == 0 or n_l == 0:
            ...
            for j in range(tp.shape[1]):
                ...
                if plot and j == 0:
                    py.append(np.interp(px, mrec, mpre))  # precision at mAP@1.0
...
def plot_pr_curve(px, py, ap, save_dir='pr_curve.png', names=()):
    ...
    ax.plot(px, py.mean(1), linewidth=3, color='blue', label='all classes %.3f mAP@1.0' % ap[:, 0].mean())
